...
static struct rt5647_init_reg init_list[] = {
	...
	{ RT5647_PRIV_INDEX	, 0x0035 },
	{ RT5647_PRIV_DATA	, 0x7155 },
#if 0
	...
	{ RT5647_SPK_L_MIXER	, 0x003e },/* DAC1/2 -> SPKVOL */
	{ RT5647_SPK_R_MIXER	, 0x003e },/* DAC1/2 -> SPKVOL */
	{ RT5647_SPO_MIXER	, 0xf807 },/* SPKVOL -> SPO */
#endif
 ...
 */
 /*+++ ASUS_BSP : Eric */
int rt5647_headset_detect_sr(struct snd_soc_codec *codec, int jack_insert)
{
	...
	printk("%s: jack_type: %s\n", __func__,
		jack_type == SND_JACK_HEADSET ? "headset" : (jack_type == SND_JACK_HEADPHONE ? "headphone" : "none"));
	return jack_type;
}
EXPORT_SYMBOL(rt5647_headset_detect_sr);
/*--- ASUS_BSP : Eric */
...
int rt5647_headset_detect(struct snd_soc_codec *codec, int jack_insert)
{
	...
	if (jack_insert) {
		...
		snd_soc_write(codec, RT5647_CJ_CTRL1, 0x0582);
		snd_soc_write(codec, RT5647_CJ_CTRL2, 0x18a7);
		snd_soc_write(codec, RT5647_JD_CTRL3, 0x00b0);
		msleep(300);
...
int rt5647_check_irq_event(struct snd_soc_codec *codec)
{
	...
	case 0x0: /* plug */
		...
		case SND_JACK_HEADSET:
			rt5647->btn_type = rt5647_button_detect(codec);
			if (rt5647->btn_type) {
				pr_debug("Button Press\n");
				ret = RT5647_BP_EVENT;
				}
			else {
...
/* ASUS_BSP Paul +++ */
void rt5647_update_spk_drc(void)
{
	if (g_is_spk_on) {
...
EXPORT_SYMBOL(rt5647_update_spk_drc);
/* ASUS_BSP Paul --- */
...
static const char *rt5647_spo_gain_ratio[] = {"-6dB", "-4.5dB", "-3dB", "-1.5dB",
	"0dB", "0.83dB", "1.58dB", "2.22dB"};

static const SOC_ENUM_SINGLE_DECL(
	rt5647_spo_gain_ratio_enum, RT5647_SPO_CLSD_RATIO,
	RT5647_SPO_CLSD_RATIO_SFT, rt5647_spo_gain_ratio);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (rt5647_HW_ID <= HW_ID_SR1)
		rt5647_headset_detect_sr(codec, 1);
	else
...
static const struct snd_kcontrol_new rt5647_snd_controls[] = {
	...
	SOC_ENUM("Speaker Boost", rt5647_spo_gain_ratio_enum),
...
static int rt5647_adc_eq_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = w->codec;
	struct rt5647_priv *rt5647 = snd_soc_codec_get_drvdata(codec);
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (/*is_vr_active() ||*/ (get_audiomode() == 2))
			return 0;

		if (rt5647->jack_type == SND_JACK_HEADSET) {
			pr_debug("%s : HS_MIC_ADC\n", __func__);
			rt5647_update_eqmode(codec, EQ_CH_HS_MIC_ADC, HS_MIC_ADC);
		} else {
			pr_debug("%s : MAIN_MIC_ADC\n", __func__);
		}
		break;

	case SND_SOC_DAPM_POST_PMD:
		pr_debug("%s : RESET_INPUT\n", __func__);
		rt5647_update_eqmode(codec, EQ_CH_RESET_INPUT, RESET_INPUT);
		break;
...
static const struct snd_soc_dapm_widget rt5647_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY("MCLK Detect", RT5647_GEN_CTRL3,
		9, 0, NULL, 0),
	SND_SOC_DAPM_SUPPLY_S("ADC EQ", 2, SND_SOC_NOPM, 0, 0,
		rt5647_adc_eq_event, SND_SOC_DAPM_PRE_PMU |
...
*/
	...
	{ "dac stereo1 filter", NULL, "MCLK Detect" },
	{ "dac mono left filter", NULL, "MCLK Detect" },
	{ "dac mono right filter", NULL, "MCLK Detect" },

	{ "adc stereo1 filter", NULL, "ADC EQ" },
...
// ASUS_BSP Paul +++
static ssize_t rt5647_codec_status_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct rt5647_priv *rt5647 = i2c_get_clientdata(client);
	...
	val = snd_soc_read(codec, RT5647_VENDOR_ID);

	if (val < 0) {
		cnt = sprintf(buf, "0\n");
		printk("%s: failed to read the codec status %d\n", __func__, val);
	} else {
		cnt = sprintf(buf, "1\n");
		printk("%s: read the codec status %d successfully\n", __func__, val);
	}

	return cnt;
}

static DEVICE_ATTR(codec_status, 0444, rt5647_codec_status_show, NULL);
// ASUS_BSP Paul ---
...
static void jd_check_handler(struct work_struct *work)
{
	...
	if (val == 0x20) {
		...
		/* use different type headset detect */
		if (rt5647_HW_ID <= HW_ID_SR1)
			rt5647_headset_detect_sr(codec, 0);
		else
...
static int rt5647_probe(struct snd_soc_codec *codec)
{
	...
	printk("[Progress][rt5647] Probe starts\n"); /* ASUS_BSP Paul +++ */

	/*ASUS_BSP : Eric*/
	rt5647_HW_ID = Read_HW_ID();
	/*ASUS_BSP : Eric*/
	...
	snd_soc_update_bits(codec, RT5647_IL_CMD, 0x0007, 0x0004);
#endif
	...
	ret = device_create_file(codec->dev, &dev_attr_codec_status);
	if (ret != 0) {
		dev_err(codec->dev,
			"Failed to create codec_status sysfs files: %d\n", ret);
		return ret;
	}
	// ASUS_BSP Paul ---

	printk("[Progress][rt5647] Probe ends\n"); /* ASUS_BSP Paul +++ */
