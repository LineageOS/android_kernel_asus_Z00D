...
/* ASUS_BSP Paul ---*/
/*+++ ASUS_BSP : Eric */
extern int ASUS_GET_ADC_VALUE(int index);
extern int ASUS_UPDATE_GPADC(void);
/*--- ASUS_BSP : Eric */
/*+++ ASUS_BSP : Eric +++*/
...
#define GPIO_SPK_AMP_EN 79
struct ctp_mc_private *g_ctx;
extern struct snd_soc_codec *rt5647_codec;
int g_DebugMode = 1;
/* ASUS_BSP Paul ---*/
...
static struct snd_soc_jack_gpio hs_gpio[] = {
	...
	[CTP_BTN_GPIO] = {
		.name = "CODEC_INT_N",
		.report = SND_JACK_HEADSET | SND_JACK_BTN_0 | SND_JACK_BTN_1 | SND_JACK_BTN_2  | SND_JACK_BTN_3 ,
		.debounce_time = 10,
			...
			struct snd_kcontrol *k, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		...
		gpio_direction_output(GPIO_SPK_AMP_EN, 1); /* ASUS_BSP Paul +++ */
	} else {
		/*Disable  IHFAMP_SD_N  GPIO */
		ret = intel_scu_ipc_iowrite8(GPIOHVCTL, GPIO_AMP_OFF);
		if (ret)
			pr_err("write of  failed, err %d\n", ret);
		gpio_direction_output(GPIO_SPK_AMP_EN, 0); /* ASUS_BSP Paul +++ */
	}
	return 0;
}
...
*/
	...
	if (status == SND_JACK_HEADSET) {
		...
		mask = SND_JACK_HEADSET | SND_JACK_BTN_0 | SND_JACK_BTN_1 | SND_JACK_BTN_2  | SND_JACK_BTN_3  ;
	}
...
void headset_remove_poll(struct work_struct *work)
{
	...
	struct ctp_mc_private *ctx =
		container_of(jack, struct ctp_mc_private, ctp_jack);
	int enable, status;
	unsigned int mask = SND_JACK_HEADSET | SND_JACK_BTN_0 | SND_JACK_BTN_1 | SND_JACK_BTN_2  | SND_JACK_BTN_3;
/*
	enable = gpio_get_value(gpio->gpio);
	if (gpio->invert)
		enable = !enable;
...
*/
/*+++ ASUS_BSP Eric */
int ASUS_GET_BUTTON_TYPE(void)
{
	int adc_value;
	ASUS_UPDATE_GPADC();
	adc_value = ASUS_GET_ADC_VALUE(4);
	printk("[Audio][ADC]ADC_VALUE[4] = %d\n", adc_value);

	if (adc_value <= 50)
		return SND_JACK_BTN_0;
	else if (adc_value <= 82 && adc_value > 50)
		return SND_JACK_BTN_3;
	else if (adc_value <= 140 && adc_value > 85)
		return SND_JACK_BTN_1;
	else if (adc_value <= 275 && adc_value > 155)
...
int ctp_soc_jack_gpio_detect_bp(void)
{
	...
	int ret = jack->status;
	/*+++ ASUS_BSP : Eric +++ */
	struct rt5647_priv *rt5647 = snd_soc_codec_get_drvdata(rt5647_codec);
	int button_type = 0;
	/*--- ASUS_BSP : Eric +++ */

	/*wake lock for 3 sec for sure event is reported*/
	wake_lock_timeout(ctx->jack_wake_lock,
	...
	if (ctp_HW_ID <= HW_ID_SR1) {
		...
		case 0x2000: /* up */
				button_type = SND_JACK_BTN_1;
				break;
		case 0x0400: /* center */
				button_type = SND_JACK_BTN_0;
				break;
		case 0x0080: /* down */
				button_type = SND_JACK_BTN_2;
				break;
		default:
				button_type = 0x0;
...
static ssize_t audio_debug_proc_write(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
	...
	if (strncmp(messages, "1", 1) == 0) {
		gpio_direction_output(GPIO_HS_PATH, 0); /* enable uart log, disable audio */
		g_DebugMode = 1;
		ctp_soc_jack_gpio_detect_bp();
		printk("[Audio][Debug] Audio debug mode!!\n");
	} else if (strncmp(messages, "0", 1) == 0) {
		gpio_direction_output(GPIO_HS_PATH, 1); /* disable uart log, enable audio */
		g_DebugMode = 0;
		ctp_soc_jack_gpio_detect_bp();
		...
		value = snd_soc_read(rt5647_codec, reg);
		printk("[Audio][rt5647] read register reg[0x%x]=[0x%x]\n", reg, value);
	} else if (strncmp(messages, "write", strlen("write")) == 0) {
		unsigned int reg, value;
		sscanf(messages + 6, "%x %x", &reg, &value);
		snd_soc_write(rt5647_codec, reg, value);
		value = snd_soc_read(rt5647_codec, reg);
		printk("[Audio][rt5647] write register reg[0x%x]=[0x%x]\n", reg, value);
	} else if (strncmp(messages, "update", strlen("update")) == 0) {
		unsigned int reg, mask, value;
		sscanf(messages + 7, "%x %x %x", &reg, &mask, &value);
		snd_soc_update_bits(rt5647_codec, reg, mask, value);
		value = snd_soc_read(rt5647_codec, reg);
		printk("[Audio][rt5647] update register reg[0x%x]=[0x%x]\n", reg, value);
	} else if (strncmp(messages, "dump", strlen("dump")) == 0) {
		unsigned int val;
		int i;
		for (i = 0; i <= RT5647_VENDOR_ID2; i++) {
			val = snd_soc_read(rt5647_codec, i);
			if (!val)
				continue;
			printk("[Audio][rt5647] dump register reg[0x%x]=[0x%x]\n", i, val);
		}
	}
						...
						bool jack_supported)
{
	...
	ret = snd_soc_jack_new(codec, "Intel MID Audio Jack",
			SND_JACK_HEADSET | SND_JACK_BTN_0 | SND_JACK_BTN_1 | SND_JACK_BTN_2  | SND_JACK_BTN_3, &ctx->ctp_jack);
	if (ret) {
	...
	ret = snd_jack_set_key((&ctx->ctp_jack)->jack,
		SND_JACK_HEADSET | SND_JACK_BTN_0 | SND_JACK_BTN_1 | SND_JACK_BTN_2, BTN_MISC);
	*/
	/*+++ ASUS_BSP : Eric */
	snd_jack_set_key((&ctx->ctp_jack)->jack, SND_JACK_HEADSET | SND_JACK_BTN_0 , KEY_MEDIA);
	snd_jack_set_key((&ctx->ctp_jack)->jack, SND_JACK_HEADSET | SND_JACK_BTN_1 , KEY_VOLUMEUP);
	snd_jack_set_key((&ctx->ctp_jack)->jack, SND_JACK_HEADSET | SND_JACK_BTN_2 , KEY_VOLUMEDOWN);
	snd_jack_set_key((&ctx->ctp_jack)->jack, SND_JACK_HEADSET | SND_JACK_BTN_3 , KEY_VOICE_ASSIST);
	/*--- ASUS_BSP : Eric*/
	/* Disable Button_press interrupt if no Headset */
...
static int snd_ctp_mc_probe(struct platform_device *pdev)
{
	...
	printk("[Progress][ctp_audio] Probe starts\n"); /* ASUS_BSP Paul +++ */
	...
	// ASUS_BSP Paul +++
	ret_val = gpio_request(GPIO_SPK_AMP_EN, "SPK_EN");
	if (ret_val)
		pr_err("%s: Failed to request gpio SPK_EN %d\n", __func__, GPIO_SPK_AMP_EN);
	...
	printk("[Progress][ctp_audio] Probe ends\n"); /* ASUS_BSP Paul +++ */
