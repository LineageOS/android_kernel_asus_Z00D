...
int log_wakeup_irq(void)
{
	unsigned int irr = 0, vector = 0;
	int offset = 0, irq = 0, return_value = 0;
		...
		while (irr) {
			...
			if (desc) {
				if ((desc->action) && (desc->action->name))
					szName = desc->action->name;
				else if (desc->name)
					szName = desc->name;
			}

			char szMessage[200];
			snprintf(szMessage, sizeof(szMessage), "[PM] Wakeup from IRQs, Name: %s, IRQ: %d.\n", szName, irq);
			printk(szMessage);
			/* ZE500CL001E */
...
{
	/* ZE500CL001S */
	pr_info("[Progress][PMU] Probe starts\n");
	/* ZE500CL001E */
	int ret;
...
#endif
	/* ZE500CL001S */
	pr_info("[Progress][PMU] Probe ends\n");
	/* ZE500CL001E */
	return 0;
