...
static void log_wakeup_source(int source)
{
	...
	/* ZE500CL001S */
	char *szDevName = "(null)";
	int nLSS = source - mid_pmu_cxt->pmu1_max_devs;
	int nWakeUpIRQ = GetWakeUpIRQ();
	int nGPIO = -1;
	struct irq_desc *pIRQDesc = irq_to_desc(nWakeUpIRQ);
	struct irqaction *pIRQAction = NULL;
	char szMessage[200];
	char szAsusEvtMsg[200];

	if (pIRQDesc) {
		if (pIRQDesc->action) {
			pIRQAction = pIRQDesc->action;

			if (pIRQAction->name)
				szDevName = pIRQAction->name;
		}
	}
	...
	case PMU_GPIO0_LSS_39:
		nGPIO = GetGPIOInfo();
		snprintf(szMessage, sizeof(szMessage), "[PM] Wakeup from GPIOs, Name: %s, IRQ: %d, LSS: %d, GPIO: %d.\n", szDevName, nWakeUpIRQ, nLSS, nGPIO);
		/* +++ Fixed format for log parser, DO NOT MODIFY +++ */
		snprintf(szAsusEvtMsg, sizeof(szAsusEvtMsg), "[PM] GPIO triggered: %d\n", nGPIO);
		/* --- Fixed format for log parser, DO NOT MODIFY --- */
		break;

	default:
		snprintf(szMessage, sizeof(szMessage), "[PM] Wakeup from IRQs, Name: %s, IRQ: %d, LSS: %d.\n", szDevName, nWakeUpIRQ, nLSS);
		/* +++ Fixed format for log parser, DO NOT MODIFY +++ */
		snprintf(szAsusEvtMsg, sizeof(szAsusEvtMsg), "[PM] IRQs triggered: %d\n", nWakeUpIRQ);
		/* --- Fixed format for log parser, DO NOT MODIFY --- */
		break;
	}
	printk(szMessage);
	ASUSEvtlog(szAsusEvtMsg);

	/* Reset PowerKey triggered resume status */
	if (nLSS != PMU_PWR_BUTTON_LSS_38)
		SetPowerKeyResumeStarted(false);
...
int pmu_get_wake_source(void)
{
	...
	while (!wake0 && !wake1) {
		if (!timeoutcount || log_wakeup_irq() == 2) {
			/*log_wakeup_irq();*/
