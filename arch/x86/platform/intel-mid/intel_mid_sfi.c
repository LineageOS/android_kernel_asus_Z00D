...
#endif /* CONFIG_ZE500CL || CONFIG_PF450CL */
...
static int __init sfi_parse_oemr(struct sfi_table_header *table)
{
	...
	SKU_ID = pentry->project_id;
	FCAM_ID = pentry->FCAM_id;
	if (pentry->project_id == PROJ_ID_PF450CL) {
		PROJECT_ID = PROJ_ID_PF450CL;
	} else if ((pentry->project_id == PROJ_ID_ZE500CL)) {
		PROJECT_ID = PROJ_ID_ZE500CL;
	}
	/*Return */
	PROJECT_ID = PROJ_ID_ZE500CL;
	...
	if (PROJECT_ID == PROJ_ID_ZE500CL || PROJECT_ID == PROJ_ID_PF450CL) {
#if 0
		switch (HARDWARE_ID) {
		case HW_ID_EVB:
			pr_info("Hardware VERSION = EVB\n");
...
static char *hardware_id_str;
module_param(hardware_id_str, charp, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(HW_VERSION_STR, "HW_ID judgement");

static char *IFWI_str;
module_param(IFWI_str, charp, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(IFWI_VERSION_STR, "IFWI version judgement");
#define IFWI_VER_LEN 128
extern char IFWI_VER[IFWI_VER_LEN];
int Read_HW_ID(void)
{
...
static char *sku_str;
module_param(sku_str, charp, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(SKU_VERSION_STR, "SKU_ID judgement");
#define SKU_VER_LEN 128
char SKU_VER[SKU_VER_LEN];

int Read_SKU_Version(void)
{
	sku_str = SKU_VER;
	switch (SKU_ID) {
	case 0:
		sku_str = "ZE500CL_ASIA_TW";
		break;
	case 1:
		sku_str = "ZE500CL_EU_US";
		break;
	default:
		sku_str = "NA";
		break;
	}
	printk("%s SKU_ID = 0x%x \n", SKU_VER, SKU_ID);

	return SKU_ID;
}
EXPORT_SYMBOL(Read_SKU_Version);

int Read_FCAM_ID(void)
{
	printk("FCAM_ID = 0x%x \n", FCAM_ID);
	return FCAM_ID;
}
EXPORT_SYMBOL(Read_FCAM_ID);

/* ASUS_BSP+++sr_Huang Hall Sensor */
static struct platform_device Hall_sensor_device = {
.name = "hall_sensor",
.id = 0,
};
static struct platform_device *Hall_sensor_devices[] = {
	&Hall_sensor_device,
};
