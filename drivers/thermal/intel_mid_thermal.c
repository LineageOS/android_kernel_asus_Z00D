...
/*+++ ASUS_BSP : Eric */
void *g_therm_adc_handle;
int *g_cached_values;

int ASUS_GET_ADC_VALUE(int index)
{
	if (g_cached_values == NULL)
		return -1;
	else
		return g_cached_values[index];
}

int ASUS_UPDATE_GPADC(void)
{
	if ((g_therm_adc_handle == NULL)  ||
		(g_cached_values == NULL))
		return -1;
	else
		 return get_gpadc_sample(g_therm_adc_handle, 2,
					g_cached_values);
}
/*--- ASUS_BSP : Eric */
static int mid_read_temp(struct thermal_zone_device *tzd, long *temp)
{
	...
	pr_debug("[thermal]mid_read_temp , index = %d\n", indx);
	...
	time_after(jiffies, platforminfo->cacheinfo.last_updated + HZ)) {
		...
		/*+++ ASUS_BSP : Eric */
		g_therm_adc_handle = platforminfo->therm_adc_handle ;
		g_cached_values = platforminfo->cacheinfo.cached_values;
		/*--- ASUS_BSP : Eric */
	}
	...
	pr_debug("[thermal]mid_read_temp , index = %d , cached_value = %d\n", indx, platforminfo->cacheinfo.cached_values[indx]);
	if (ret)
		goto exit;
...
static int mid_therm_rpmsg_probe(struct rpmsg_channel *rpdev)
{
	...
	printk("[Progress][%s] Probe starts\n", DRIVER_NAME);
	if (rpdev == NULL) {
