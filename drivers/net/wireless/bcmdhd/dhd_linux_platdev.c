...
extern int wifi_get_mac_addr_intel(unsigned char *buf);
struct wifi_platform_data dhd_wlan_control = {
	.set_power = bcmsdh_sdmmc_set_power,
	.get_mac_addr = wifi_get_mac_addr_intel,
};
...
/* ASUS_BSP+++ "for wlan wakeup trace" */
extern int wakeup_irq_wlan_num(int irq);
/* ASUS_BSP--- "for wlan wakeup trace" */
...
int wifi_platform_get_irq_number(wifi_adapter_info_t *adapter, unsigned long *irq_flags_ptr)
{
	...
	printk("[wlan]: %s: adapter->irq_num = %d.\n", __FUNCTION__, adapter->irq_num);

	/* ASUS_BSP+++ "for wlan wakeup trace" */
	pin_num = adapter->irq_num;
	irq_num = gpio_to_irq(pin_num);
	printk("[wlan]: wifi_platform_get_irq_number, (pin_num=%d, irq_num=%d).\n", pin_num, irq_num);
	wakeup_irq_wlan_num(irq_num);
	/* ASUS_BSP--- "for wlan wakeup trace" */
...
bool wifi_platform_irq_is_fastirq(wifi_adapter_info_t *adapter)
{
	...
	printk("[wlan]: %s: plat_data->use_fast_irq = %d\n", __FUNCTION__, plat_data->use_fast_irq);
...
int wifi_platform_get_mac_addr(wifi_adapter_info_t *adapter, unsigned char *buf)
{
	...
	if (plat_data->get_mac_addr) {
		...
		DHD_ERROR(("fail to get mac addr from mac.txt, will use default mac addr\n"));
	}
	return -EOPNOTSUPP;
}
...
static int wifi_plat_dev_drv_probe(struct platform_device *pdev)
{
	...
	((struct wifi_platform_data *)(adapter->wifi_plat_data))->get_mac_addr = wifi_get_mac_addr_intel;
