...
int dhd_customer_oob_irq_map(void *adapter, unsigned long *irq_flags_ptr)
{
	...
	printk("[wlan]: %s: gpio_request: host_oob_irq = %d\n", __FUNCTION__, host_oob_irq);
	...
	printk("[wlan]: %s: gpio_to_irq: host_oob_irq = %d\n", __FUNCTION__, host_oob_irq);
#else
...
int wifi_get_mac_addr_intel(unsigned char *buf)
{
	...
	char fname[] = "/factory/mac.txt";
	char autogen_fname[] = "/factory/autogen_mac.txt";

	WL_ERROR(("%s Enter\n", __FUNCTION__));

	/*ASUS_BSP_WIFI ++++ check if mac.txt/autogen_mac.txt exist*/
	fp = dhd_os_open_image(fname);
	if (fp == NULL) {
		WL_ERROR(("%s: unable to open %s\n", __FUNCTION__, fname));

		fp = dhd_os_open_image(autogen_fname);
		if (fp == NULL) {
			WL_ERROR(("%s: unable to open %s\n", __FUNCTION__, autogen_fname));
			return 1;
		} else
			WL_ERROR(("%s: success to open %s\n", __FUNCTION__, autogen_fname));
	} else {
		WL_ERROR(("%s: success to open %s\n", __FUNCTION__, fname));
	...
	WL_ERROR(("%s: read from file mac address: %02x:%02x:%02x:%02x:%02x:%02x\n",
