	...
	uint16 func, uint bustype, void *regsva, osl_t * osh, void *sdh)
{
	...
	printk("[Progress][dhdsdio] Probe starts \n");
	DHD_TRACE(("%s: Enter\n", __FUNCTION__));
	...
	printk("[Progress][dhdsdio] Probe ends \n");
	return bus;
...
forcereturn:
	...
	printk("[Progress][dhdsdio] Probe fails \n");
	return NULL;
}
...
_dhdsdio_download_firmware(struct dhd_bus *bus)
{
	...
	if ((bus->fw_path != NULL) && (bus->fw_path[0] != '\0')) {
		if (dhdsdio_download_code_file(bus, bus->fw_path)) {
			DHD_ERROR(("[wlan]: %s: firmware download failed, firmware path=%s\n",
	...
	if (dhdsdio_download_nvram(bus)) {
		DHD_ERROR(("[wlan]: %s: nvram download failed, nvram path=%s\n",
			   __FUNCTION__, bus->fw_path));
...
dhd_bus_devreset(dhd_pub_t *dhdp, uint8 flag)
{
	...
	bus = dhdp->bus;
	DHD_TRACE_HW4(("[wlan]: %s: firmware path=%s, nvram path=%s\n",
		__FUNCTION__, bus->fw_path, bus->nv_path));
