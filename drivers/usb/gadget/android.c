...
static int g_acm_enable;
static char g_functions[256];
/* ASUS_BSP--- BennyCheng "enable acm by engineer mode for AT&T requirement" */
			       ...
			       const char *buff, size_t size)
{
	...
	/* ASUS_BSP+++ BennyCheng "enable acm by engineer mode for AT&T requirement" */
	if (g_acm_enable) {
		printk("android_usb: skip usb functions operation when acm function enabled (%s)\n", buff);
		return size;
	}
	/* ASUS_BSP--- BennyCheng "enable acm by engineer mode for AT&T requirement" */
	...
	/* ASUS_BSP+++ BennyCheng "enable acm by engineer mode for AT&T requirement" */
	if (strstr(buf, "acm") != NULL) {
		pr_err("android_usb: only support switch acm function by asus tool (%s)", buf);
		strlcpy(buf, "adb", sizeof("adb"));
	} else {
		strlcpy(buf, buff, sizeof(buf));
	}
	strlcpy(g_functions, buf, sizeof(buf));

	b = strim(buf);

	printk("android_usb: switch usb function to %s\n", b);
	/* ASUS_BSP--- BennyCheng "enable acm by engineer mode for AT&T requirement" */
			    ...
			    const char *buff, size_t size)
{
	...
	/* ASUS_BSP+++ BennyCheng "enable acm by engineer mode for AT&T requirement" */
	if (g_acm_enable) {
		printk("android_usb: skip usb enable/disable operation when acm function enabled\n");
		return size;
	}
	/* ASUS_BSP--- BennyCheng "enable acm by engineer mode for AT&T requirement" */
	...
	if (enabled && !dev->enabled) {
		...
		/* ASUS_BSP+++ BennyCheng "enable acm by engineer mode for AT&T requirement" */
		if (!strcmp(g_functions, "rndis,acm,adb")) {
			cdev->desc.idVendor = __constant_cpu_to_le16(0x8087);
			cdev->desc.idProduct = __constant_cpu_to_le16(0x0a1b);
		}
		printk("android_usb: set vid: %04x, pid: %04x for %s functions\n", cdev->desc.idVendor, cdev->desc.idProduct, g_functions);
		/* ASUS_BSP--- BennyCheng "enable acm by engineer mode for AT&T requirement" */
		list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
			if (f->enable)
				f->enable(f);
...
/* ASUS_BSP+++ BennyCheng "enable acm by engineer mode for AT&T requirement" */
static ssize_t acm_show(struct device *pdev, struct device_attribute *attr,
			   char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", g_acm_enable);
}
static ssize_t acm_store(struct device *pdev, struct device_attribute *attr,
			    const char *buff, size_t size)
{
	sscanf(buff, "%d", &g_acm_enable);
	return size;
}
/* ASUS_BSP--- BennyCheng "enable acm by engineer mode for AT&T requirement" */
...
/* ASUS_BSP+++ BennyCheng "enable acm by engineer mode for AT&T requirement" */
static DEVICE_ATTR(acm_enable, S_IRUGO | S_IWUSR, acm_show, acm_store);
/* ASUS_BSP--- BennyCheng "enable acm by engineer mode for AT&T requirement" */
