...
#endif
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][porting] Separate original OTG driver with the one dedicated for padfone" */
#include <linux/usb/penwell_otg-padfone.h>
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][porting] Separate original OTG driver with the one dedicated for padfone" */
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][fix] Support host charging for stress test" */
static int g_allow_host_ext_power;
static struct delayed_work delay_detect_host_charge_work;
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][fix] Support host charging for stress test" */

/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][spec] Add proc file host_charging_mode (ac/unknown) in case the current is not enough for charging" */
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][fix] Skip show_langwell_udc in host mode" */
bool asus_otg_is_host_mode(void)
{
	struct penwell_otg *pnw = the_transceiver;
	return pnw->host_mode;
}
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][fix] Skip show_langwell_udc in host mode" */

/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Add asus_otg_set_charger function for charger control and support UNKNOWN charger" */
static int g_charger_type;
static int g_charger_state;
static struct work_struct set_usb_chg_work;
static struct delayed_work set_unknown_chg_work;
...
static void asus_otg_set_charger(int usb_state)
{
	...
	if (usb_state == g_charger_state) {
		...
		printk("[usb_otg] skip %s (already set)\n", charger_event_to_str(usb_state));
	} else if (!pnw->host_mode) {
#if defined(CONFIG_ME372CG_BATTERY_SMB345)
		if ((USB_IN == usb_state || AC_IN == usb_state || UNKNOWN_IN == usb_state) && !hsm->b_sess_vld) {
			printk("[usb_otg] skip %s (%d)\n", charger_event_to_str(usb_state), hsm->b_sess_vld);
		} else {
			printk("[usb_otg] %s(%s)\n", __func__, charger_event_to_str(usb_state));
			g_charger_state = usb_state;
			setSMB345Charger(usb_state);
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Add ASUS event log for set_chg_mode" */
...
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][Spec] Add ASUS event log for set_chg_mode" */
		...
		if (ENABLE_5V == usb_state || DISABLE_5V == usb_state) {
#if defined(CONFIG_ME372CG_BATTERY_SMB345)
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][fix] Support host charging for stress test" */
			if (ENABLE_5V == usb_state && g_allow_host_ext_power && hsm->b_sess_vld) {
				printk("[usb_otg] External power source detected - skip ENABLE_5V\n");
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][spec] Add proc file host_charging_mode (ac/unknown) in case the current is not enough for charging" */
				g_charger_state = g_host_charging_mode;
				setSMB345Charger(g_host_charging_mode);
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][spec] Add proc file host_charging_mode (ac/unknown) in case the current is not enough for charging" */
			} else {
				printk("[usb_otg] %s\n", charger_event_to_str(usb_state));
				g_charger_state = usb_state;
...
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][fix] Support host charging for stress test" */
			...
			dev_info(pnw->dev, "ignore to set charger in host mode (%s)\n", charger_event_to_str(usb_state));
		}
	}
}

void asus_otg_reset_notify(void)
{
	printk("[usb_otg] receive usb reset event (%d)(%d)\n", g_charger_type, g_charger_state);
	if (CHRG_SDP == g_charger_type && USB_IN != g_charger_state) {
		schedule_work(&set_usb_chg_work);
	}
}

static void asus_otg_set_usb_chg_work(struct work_struct *w)
{
	printk("[usb_otg] set USB_IN by reset event (%d)(%d)\n", g_charger_type, g_charger_state);

	cancel_delayed_work_sync(&set_unknown_chg_work);
	asus_otg_set_charger(USB_IN);
}

static void asus_otg_set_unknown_chg_work(struct work_struct *w)
{
...
/* static variables will be initialized to be 0s or NULLs */
static int g_host_none_mode;
static int g_keep_power_on;
static int g_suspend_delay_work_run;
const char *usb_device_list[] = {"/Removable/USBdisk1", "/sys/class/net/eth0", "/sys/class/sound/card3"};
...
static struct wake_lock early_suspend_work_wlock;
...
static void asus_otg_host_auto_switch(enum host_auto_sw req_mode)
{
	...
	switch (req_mode) {
	case HOST_AUTO_NONE:
		printk("[usb_otg] switch to auto none mode (%d)\r\n", iotg->otg.state);
		hsm->id = ID_B;
		hsm->b_sess_vld = 0;
		hsm->b_sess_end = 1;
		hsm->a_sess_vld = 0;
		...
		break;
	case HOST_AUTO_HOST:
		printk("[usb_otg] switch to auto host mode (%d)\r\n", iotg->otg.state);
		hsm->id = ID_A;
...
void asus_otg_host_mode_prepare(void)
{
	g_suspend_delay_work_run = 0;
	g_keep_power_on = 0;
	g_host_none_mode = 0;
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Add asus_otg_set_charger function for charger control and support UNKNOWN charger" */
	g_charger_type = CHRG_UNKNOWN;
	cancel_work_sync(&set_usb_chg_work);
	cancel_delayed_work_sync(&set_unknown_chg_work);
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][Spec] Add asus_otg_set_charger function for charger control and support UNKNOWN charger" */
}
...
static void asus_otg_late_resume_work(struct work_struct *w)
{
	...
	if (pnw->host_mode && g_host_none_mode)
		asus_otg_host_auto_switch(HOST_AUTO_HOST);
	else
		printk("[usb_otg] skip host auto switch (host_mode=%d)(none_mode=%d)(otg_state=%d)\n", pnw->host_mode, g_host_none_mode, iotg->otg.state);
...
static void asus_otg_early_suspend(struct early_suspend *h)
{
	...
	dev_info(pnw->dev, "%s()+++ , host_mode=%d\n", __func__, pnw->host_mode);
...
/*	No need to turn on 5V manually when there is no pad for balance mode.
	Check the note in asus_otg_late_resume function.
extern void SetMicrop5VandVbus();
*/
...
/* 	Note:
		We filter out all setCharger actions in pad, to let charger control the on/off of 5V (balance mode).
		That's why we have to recover 5V when there is pad.
		However, when there is no pad, in the process of switching back from NONE mode to HOST mode,
		5V will always be turned on (not filtered), and as a result, we don't have to recover 5V when there is no pad.
	BSP david; switch Vbus on, because Vbus may be switched off in suspend when phone not charging
	SetMicrop5VandVbus();
*/
...
{
	struct penwell_otg	 *pnw = the_transceiver;
	printk("[usb_otg] %s(), host_mode=%d, none_mode=%d\n", __func__, pnw->host_mode, g_host_none_mode);
	if (pnw->host_mode) {
		if (!g_host_none_mode) {
			g_suspend_delay_work_run = 1;
			asus_otg_host_auto_switch(HOST_AUTO_NONE);
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][other] Add manual mode switch support" */
static int id_state;
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][other] Add manual mode switch support" */
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][fix] Fix crash on boot with OTG cable" */
static struct work_struct check_at_boot;
static int g_otg_check_at_boot;
static int g_otg_delayed_mode_to_write = USB_MODE_AUTO;
...
static void asus_otg_check_at_boot(struct work_struct *w)
{
	...
	if (!g_otg_check_at_boot) {
		...
		printk("[usb_otg] boot_completed, current_id=%d, mode_to_write=%d\n", id_state, g_otg_delayed_mode_to_write);
		if (g_otg_delayed_mode_to_write != USB_MODE_AUTO) {
			pnw->otg_mode = g_otg_delayed_mode_to_write;
			asus_otg_mode_switch(g_otg_delayed_mode_to_write);
		} else {
			pnw->otg_mode = USB_MODE_AUTO;
			asus_otg_mode_switch(USB_MODE_AUTO);
		}
	}
}
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][fix] Fix crash on boot with OTG cable" */

/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Add a debug file for dynamic eye diagram parameter overriding" */
static int override_eye_diagram;
module_param(override_eye_diagram, int, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(override_eye_diagram, "Override Eye Diagram Parameter");
...
void asus_otg_set_eye_diagram(void)
{
	struct penwell_otg	*pnw = the_transceiver;
...
static ssize_t asus_otg_eye_diagram_write(struct file *file, const char __user *ubuf,
				size_t count, loff_t *ppos)
{
	...
	override_eye_diagram = myxtoi(buf);
	asus_otg_set_eye_diagram();
out:
	return status;
}

static int asus_otg_eye_diagram_show(struct seq_file *s, void *unused)
{
	struct penwell_otg	*pnw = the_transceiver;
	struct intel_mid_otg_xceiv *iotg = &pnw->iotg;
	int retval = 0;
	...
	retval = penwell_otg_ulpi_read(iotg, ULPI_VS1SET, &data);
	if (retval) {
		seq_printf(s, "err (param: 0x%x)(ret: %d)\n", override_eye_diagram, retval);
	} else {
		seq_printf(s, "param: 0x%x, reg: 0x%x\n", override_eye_diagram, data);
	}
...
static int asus_otg_eye_diagram_open(struct inode *inode, struct file *file)
{
	return single_open(file, asus_otg_eye_diagram_show, inode->i_private);
}

const struct file_operations asus_otg_eye_diagram_fops = {
	.open = asus_otg_eye_diagram_open,
	.read = seq_read,
	.write = asus_otg_eye_diagram_write,
	.llseek = seq_lseek,
	.release = single_release,
};
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][Spec] Add a debug file for dynamic eye diagram parameter overriding" */
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][fix] Support host charging for stress test" */
static int asus_otg_allow_host_ext_power_show(struct seq_file *s, void *unused)
{
	seq_printf(s, "%s\n", g_allow_host_ext_power ? "enable" : "disable");
	return 0;
}

static int asus_otg_allow_host_ext_power_open(struct inode *inode, struct file *file)
{
	return single_open(file, asus_otg_allow_host_ext_power_show, PDE_DATA(inode));
}

static ssize_t asus_otg_allow_host_ext_power_write(struct file *file, const char __user *ubuf,
				size_t count, loff_t *ppos)
{
	char buf[10];
	memset(buf, 0x00, sizeof(buf));
	...
	if (!strncmp(buf, "enable", 6)) {
		g_allow_host_ext_power = 1;
	} else if (!strncmp(buf, "disable", 7)) {
		g_allow_host_ext_power = 0;
	} else {
		return -EINVAL;
	}

	printk("[usb_otg] Allow host external power - %s\n", g_allow_host_ext_power ? "enabled" : "disabled");
	return count;
}

const struct file_operations asus_otg_proc_allow_host_ext_power_fops = {
	.open = asus_otg_allow_host_ext_power_open,
	.read = seq_read,
	.write = asus_otg_allow_host_ext_power_write,
	.llseek = seq_lseek,
	.release = single_release,
};
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][fix] Support host charging for stress test" */

/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][other] Add some debug files for OTG state checking" */
#if defined(CONFIG_ME372CG_BATTERY_SMB345)
bool asus_otg_vbus_out_state(void)
{
	int chg_otg_gpio = 0;

	chg_otg_gpio = get_gpio_by_name("CHG_OTG");
	if (chg_otg_gpio <= 0) {
		pr_err("failed to get \"CHG OTG\" gpio (%d)\n", chg_otg_gpio);
		return -1;
	}

	return gpio_get_value(chg_otg_gpio);
}
...
static int asus_otg_5v_output_show(struct seq_file *s, void *unused)
{
	...
	otg_power  = asus_otg_vbus_out_state();
	if (otg_power < 0) {
		seq_printf(s, "err\n");
	} else {
		seq_printf(s, "(%s)\n", otg_power ? "enable" : "disable");
	}
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Add asus_otg_set_charger function for charger control and support UNKNOWN charger" */
	if (!strncmp(buf, "enable", 6)) {
		asus_otg_set_charger(ENABLE_5V);
	} else if (!strncmp(buf, "disable", 7)) {
...
static void asus_otg_mode_switch(enum otg_usb_mode_type req_mode)
{
	struct penwell_otg	 *pnw = the_transceiver;
	struct intel_mid_otg_xceiv *iotg = &pnw->iotg;
	struct otg_hsm *hsm = &iotg->hsm;
	...
	case USB_MODE_AUTO:
		hsm->id = id_state;
		pnw->host_mode = (hsm->id == ID_A) ? 1 : 0;
		iotg->hsm.a_vbus_vld = pnw->host_mode;
		printk("[usb_otg] switch to auto mode, current id=%d\r\n", hsm->id);
		break;
	default:
				...
				size_t count, loff_t *ppos)
{
	...
	char buf[16];
	int status = count;
	enum otg_usb_mode_type req_mode;
	...
	if (!strncmp(buf, "host", 4)) {
		req_mode = USB_MODE_HOST;
	} else if (!strncmp(buf, "peripheral", 10)) {
		req_mode = USB_MODE_PERIPHERAL;
	} else if (!strncmp(buf, "none", 4)) {
		req_mode = USB_MODE_NONE;
	} else if (!strncmp(buf, "auto", 4)) {
		req_mode = USB_MODE_AUTO;
	} else {
		status = -EINVAL;
		goto out;
	}
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][fix] Fix crash on boot with OTG cable" */
	if (!g_otg_check_at_boot) {
		printk("delay asus_otg_mode_switch caused by write request, req=%s\n", buf);
		g_otg_delayed_mode_to_write = req_mode;
	} else {
		pnw->otg_mode = req_mode;
...
static struct dentry *asus_otg_debugfs_root;

static void asus_otg_debugfs_init(struct penwell_otg *pnw)
{
	asus_otg_debugfs_root = debugfs_create_dir("asus_otg", NULL);

	if (!asus_otg_debugfs_root || IS_ERR(asus_otg_debugfs_root))
		return;

/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][other] Add manual mode switch support" */
	if (!debugfs_create_file("mode", S_IRUGO | S_IWUSR,
				asus_otg_debugfs_root, pnw, &asus_otg_mode_fops))
		goto error;
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][other] Add manual mode switch support" */

	if (!debugfs_create_file("otg_5v_output", S_IRUGO | S_IWUSR,
			asus_otg_debugfs_root, pnw, &asus_otg_5v_output_fops))
		goto error;

/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Add a debug file for dynamic eye diagram parameter overriding" */
	if (!debugfs_create_file("eye_diagram", S_IRUGO | S_IWUSR,
				asus_otg_debugfs_root, pnw, &asus_otg_eye_diagram_fops))
		goto error;
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][Spec] Add a debug file for dynamic eye diagram parameter overriding" */
...
error:
	debugfs_remove_recursive(asus_otg_debugfs_root);
}

static void asus_otg_debugfs_cleanup(void)
{
	debugfs_remove(asus_otg_debugfs_root);
}
...
static int asus_otg_proc_init(struct penwell_otg *pnw)
{
	struct proc_dir_entry *proc_entry;

	asus_otg_proc_root = proc_mkdir("asus_otg", NULL);
	if (!asus_otg_proc_root) {
		return -ENODEV;
	}

	proc_entry = proc_create_data("mode", S_IRUGO |S_IWUSR, asus_otg_proc_root,
			&asus_otg_proc_mode_fops, pnw);
	if (!proc_entry) {
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][fix] Fix crash on boot with OTG cable" */
	proc_entry = proc_create_data("otg_boot_check", S_IRUGO | S_IWUSR, asus_otg_proc_root,
			&asus_otg_proc_otg_boot_check_fops, pnw);
	if (!proc_entry) {
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][spec] Add proc file host_charging_mode (ac/unknown) in case the current is not enough for charging" */
	proc_entry = proc_create_data("host_charging_mode", S_IRUGO | S_IWUSR, asus_otg_proc_root,
			&asus_otg_proc_host_charging_mode_fops, pnw);
	if (!proc_entry) {
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][fix] Support host charging for stress test" */
	proc_entry = proc_create_data("allow_host_ext_power", S_IRUGO | S_IWUSR, asus_otg_proc_root,
			&asus_otg_proc_allow_host_ext_power_fops, pnw);
	if (!proc_entry) {
		remove_proc_entry("allow_host_ext_power", asus_otg_proc_root);
		asus_otg_proc_root = NULL;
		return -ENODEV;
	}
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][fix] Support host charging for stress test" */
	proc_entry = proc_create_data("eye_diagram", S_IRUGO | S_IWUSR, asus_otg_proc_root,
			&asus_otg_eye_diagram_fops, pnw);
	if (!proc_entry) {
		remove_proc_entry("eye_diagram", asus_otg_proc_root);
		asus_otg_proc_root = NULL;
		return -ENODEV;
	}
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Add asus_otg_set_charger function for charger control and support UNKNOWN charger" */
			asus_otg_set_charger(enabled ? ENABLE_5V : DISABLE_5V);
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][Spec] Add asus_otg_set_charger function for charger control and support UNKNOWN charger" */
			kfree(evt);
			goto done;
		}
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][fix] Disable ACA detection to improve detection stability and avoid potential issue" */
#if 0
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][other] Add manual mode switch support" */
	id_state = (val32 & OTGSC_ID) ? ID_B : ID_A;
	printk("[usb_otg] switch to %s mode (boot)(bsv: %d)(id_state: %d)\r\n",
		iotg->hsm.id == ID_A ? "host" : "peripheral", iotg->hsm.b_sess_vld, id_state);
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][other] Add manual mode switch support" */
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][other] Add manual mode switch support" */
	id_state = !!(val32 & OTGSC_ID) ? ID_B : ID_A;
	if (USB_MODE_AUTO == pnw->otg_mode) {
		iotg->hsm.id = id_state;
		iotg->hsm.a_vbus_vld = (iotg->hsm.id == ID_A) ? 1 : 0;
	}
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][other] Add manual mode switch support" */
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Add a debug file for dynamic eye diagram parameter overriding" */
	if (override_eye_diagram) {
		retval = penwell_otg_ulpi_write(iotg, ULPI_VS1SET, override_eye_diagram);
		if (retval)
			dev_warn(pnw->dev,
				"eye diagram optimize failed with ulpi failure (0x%x)(%d)\n", override_eye_diagram, retval);
	} else {
	...
	retval = penwell_otg_ulpi_read(iotg, ULPI_VS1SET, &data);
	if (retval) {
		dev_warn(pnw->dev, "failed to read get eye diagram parameter (%d)\n", retval);
	} else {
		dev_info(pnw->dev, "eye diagram param: 0x%x\n", data);
	}
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][Spec] Add a debug file for dynamic eye diagram parameter overriding" */
}
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][other] Add manual mode switch support" */
			if(!id) {
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][fix] Fix crash on boot with OTG cable" */
				id_state = (int_sts & OTGSC_ID) ? ID_B : ID_A;
				if (!g_otg_check_at_boot) {
					printk("[usb_otg] not handle ID events before boot completed (%d)\n", id_state);
				} else if (USB_MODE_AUTO != pnw->otg_mode) {
					printk("[usb_otg] skipped id change int = %d, otg_mode=%d\n", id_state, pnw->otg_mode);
				} else {
					iotg->hsm.id = id_state;
					flag = 1;
					printk("[usb_otg] id change int = %d\n", id_state);
					/* Update a_vbus_valid once ID changed */
					iotg->hsm.a_vbus_vld = (iotg->hsm.id == ID_A) ? 1 : 0;
					pnw->host_mode = iotg->hsm.a_vbus_vld;
				}
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][fix] Fix crash on boot with OTG cable" */
			}
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][other] Add manual mode switch support" */
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Add asus_otg_set_charger function for charger control and support UNKNOWN charger" */
			...
			if (charger_type == CHRG_SE1) {
				...
				penwell_otg_update_chrg_cap(CHRG_SE1,
							CHRG_CURR_SE1);
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Support apple chargers (SE1)" */
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Add asus_otg_set_charger function for charger control and support UNKNOWN charger" */
				...
				schedule_delayed_work(&set_unknown_chg_work, (2000 * HZ/1000));
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Add asus_otg_set_charger function for charger control and support UNKNOWN charger" */
	...
	case OTG_STATE_B_PERIPHERAL:
		...
		if (hsm->id == ID_A) {
			...
			if (hsm->id == ID_ACA_B)
				...
				iotg->otg.state = OTG_STATE_B_IDLE;
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][fix] Fix no cable out event due to b_sess_end int come earlier" */
			if (hsm->b_sess_end) {
				dev_info(pnw->dev, "b_sess_end int come earlier, queue work again!\n");
				penwell_update_transceiver();
			}
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][fix] Fix no cable out event due to b_sess_end int come earlier" */
		...
		const struct pci_device_id *id)
{
	...
	retval = 0;
	printk("[Progress][usb_otg] Probe starts");
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Add asus_otg_set_charger function for charger control and support UNKNOWN charger" */
	INIT_WORK(&set_usb_chg_work, asus_otg_set_usb_chg_work);
	INIT_DELAYED_WORK(&set_unknown_chg_work, asus_otg_set_unknown_chg_work);
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][Spec] Add asus_otg_set_charger function for charger control and support UNKNOWN charger" */
	/* OTG common part */
	pnw->dev = &pdev->dev;
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Register early suspend notification for none mode switch" */
	...
	wake_lock_init(&early_suspend_work_wlock, WAKE_LOCK_SUSPEND, "asus_otg_early_suspend_work_wlock");
	if (!early_suspend_delay_wq)
		early_suspend_delay_wq = create_singlethread_workqueue("asus_otg_early_suspend_delay_wq");
	INIT_DEFERRABLE_WORK(&early_suspend_delay_work, asus_otg_early_suspend_delay_work);
...
/* ASUS_BSP--- LandiceFu "[ZE500CL][USBH][NA][fix] Fix crash on boot with OTG cable" */
	...
	printk("[Progress][usb_otg] Probe ends");
	return 0;
...
/* ASUS_BSP+++ LandiceFu "[ZE500CL][USBH][NA][Spec] Register early suspend notification for none mode switch" */
	...
	wake_lock_destroy(&early_suspend_work_wlock);
#if defined(CONFIG_HAS_EARLYSUSPEND)
