...
static void lnw_irq_init_hw(struct lnw_gpio *lnw)
{
	...
	/* BSP Joy Apply GP_AON_045 Lo to make CPU go into Power Saving mode +++ */
	if (lnw->chip.base == 0) {
		lnw_gpio_set(&lnw->chip, 45, 0);
	}
	/* BSP Joy Apply GP_AON_045 Lo to make CPU go into Power Saving mode --- */
}
			...
			const struct pci_device_id *id)
{
	...
	/* ZE500CL001S */
	struct irq_desc *pIRQDesc = irq_to_desc(pdev->irq);
	if (pIRQDesc) {
		if (gpio_base == 0)
			pIRQDesc->name = "GPIO AON";
		else
			pIRQDesc->name = "GPIO CORE";
	}
	/* ZE500CL001E */
