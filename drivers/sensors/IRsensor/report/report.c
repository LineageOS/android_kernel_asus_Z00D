...
void proximity_report_onoff(bool bOn)
{
	if(bOn == true) { /* Turn on */
		input_report_abs(input_dev_ps, ABS_DISTANCE, 1);
...
void light_report_onoff(bool bOn)
{
	if(bOn == false) {	/* Turn off */
		input_report_abs(input_dev_als, ABS_MISC, -1);
