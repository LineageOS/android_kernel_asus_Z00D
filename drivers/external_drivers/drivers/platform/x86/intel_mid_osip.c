...
#endif
	pr_info("[REBOOT] %s, intel_scu_ipc_write_osnib_rr = %s, id = %d\n", __func__, reboot_target_id2name(id), id);
	ret_ipc = intel_scu_ipc_write_osnib_rr(id);
	if (ret_ipc < 0)
		pr_err("%s cannot write %s reboot reason in OSNIB\n",
			__func__, target);
	ret_debug = intel_scu_ipc_read_osnib_rr(&rbt_reason_debug);
	pr_info("[REBOOT] %s, intel_scu_ipc_read_osnib_rr = %s, id = %d\n", __func__, reboot_target_id2name(rbt_reason_debug), rbt_reason_debug);
