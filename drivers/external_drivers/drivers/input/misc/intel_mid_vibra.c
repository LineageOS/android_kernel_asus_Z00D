...
#include <linux/wakelock.h>
/* ASUS BSP freddy-- register wakelock for press key-code */
/* +++ ASUS_BSP : Add vibrator wake lock */
static struct wake_lock vibrator_wake_lock;
/* --- ASUS_BSP : Add vibrator wake lock */
...
static int vibra_soc_pwm_configure(struct vibra_info *info, bool enable)
{
	...
	if (enable) {
		...
		*/
		...
		printk("[vibrator] config_vibrator on\n");
	      }
	} else { /*disable PWM block */
		if (info->gpio_pwm > 0) {
			gpio_set_value(info->gpio_pwm, 1);
			...
			printk("[vibrator] config_vibrator off\n");
	    }
	}
	return 0;
}
...
/*	printk("[vibrator] info->enabled = %d\n", info->enabled);*/
	...
	struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (kstrtol(buf, 0, &vibrator_enable))
		return -EINVAL;
/*	printk("[vibrator] vibrator_enable = %ld\n", vibrator_enable);
	printk("[vibrator] info->enabled = %d\n", info->enabled);
	printk("[vibrator] len = %d\n", len);*/
...
static DEVICE_ATTR(vibrator, S_IRUGO | S_IWUSR | S_IWGRP |
 S_IWOTH,
...
/*ASUS_BSP freddy++ for PAD ADD vibrate*/
#if (defined(CONFIG_PF450CL) || defined(CONFIG_PF451CL)) && defined(CONFIG_EEPROM_PADSTATION)
#include <linux/microp_notify.h>
#include <linux/microp_api.h>
struct vibra_info *g_info;

void  mipi_vibrator_report(void)
{
	printk("[vibrator] PAD ADD. vibrate on\n");
	printk("[vibrator] +++ wake lock for 1 sec\n");
	wake_lock_timeout(&vibrator_wake_lock, 1 * HZ);
	g_info->enable(g_info);
	msleep(500);
	printk("[vibrator] PAD ADD. vibrate off\n");
	g_info->disable(g_info);
	wake_unlock(&vibrator_wake_lock);
	printk("[vibrator] --- wake lock for 1 sec\n");
}

static int mp_vibrator_report(struct notifier_block *this, unsigned long event, void *ptr)
{
	if (event == P01_REMOVE) {
		printk("[vibrator] PAD REMOVE. vibrate off\n");
		g_info->disable(g_info);
	}
	return NOTIFY_DONE;
}

static struct notifier_block mp_vibrator_notifier = {
	.notifier_call = mp_vibrator_report,
...
struct vibra_info *mid_vibra_setup(struct device *dev, struct mid_vibra_pdata *data)
{
	...
	info->name = data->name;
/*	printk("[vibrator]info->name=%s,info->gpio_pwm=%d\n", info->name, info->gpio_pwm);*/
	info->dev = dev;
			...
			const struct pci_device_id *pci_id)
{
	...
	printk("[Progress][mid_vibra] Probe starts\n");

	/* +++ ASUS_BSP : Add vibrator wake lock */
	wake_lock_init(&vibrator_wake_lock, WAKE_LOCK_SUSPEND, "vibrator_lock");
	/* --- ASUS_BSP : Add Vibrator wake lock */
...
/*ASUS_BSP freddy++ for PAD ADD vibrate*/
#if (defined(CONFIG_PF450CL) || defined(CONFIG_PF451CL)) && defined(CONFIG_EEPROM_PADSTATION)
	ret = register_microp_notifier(&mp_vibrator_notifier);
	if (ret) {
