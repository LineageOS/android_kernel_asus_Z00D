...
long mdm_ctrl_dev_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
	...
	/* ASUS-BBSP AP side request core dump+++ */
	case 0x8746:
		pr_err (DRVNAME": AP Request Core Dump!!\n");
...
static int mdm_ctrl_module_probe(struct platform_device *pdev)
{
	...
	/* ASUS-BBSP AP side request core dump+++ */
	pr_err (DRVNAME": Try to configure AP_REQ_CORE_DUMP_GPIO (%d)\n", get_gpio_by_name("AP_REQ_CORE_DUMP"));
	ret = gpio_request_one(get_gpio_by_name("AP_REQ_CORE_DUMP"), GPIOF_DIR_OUT, "AP_REQ_CORE_DUMP_NAME");
	ret += gpio_direction_output (get_gpio_by_name("AP_REQ_CORE_DUMP"), 0);

	if (ret) {
		pr_err (DRVNAME": Unable to configure AP_REQ_CORE_DUMP_GPIO (%d)\n", get_gpio_by_name("AP_REQ_CORE_DUMP"));
		gpio_free (get_gpio_by_name("AP_REQ_CORE_DUMP"));
	}
	/* ASUS-BBSP AP side request core dump--- */
