...
extern int wakeup_print_data; /* ASUS-BBSP+ packet wakeup trace */
				...
				struct hsi_msg *msg)
{
	...
	if (intel_hsi->wakeup_packet_log) {
		...
		wakeup_print_data = 1; /* ASUS-BBSP+ packet wakeup trace */
		...
		pr_info (DRVNAME " : wake up from channel %d\n", msg->channel); /* ASUS-BBSP Log to check wakeup from which channel */
	}
...
static int hsi_pm_suspend(struct device *dev)
{
	...
	pr_err("hsi enter suspend+++\n");/*ASUS-BBSP Add hsi-ctrl suspend/resume logs+*/
	...
	pr_err("hsi enter suspend---err = %d\n", err);/*ASUS-BBSP Add hsi-ctrl suspend/resume logs+*/
	return err;
}
