...
#include <linux/gpio.h> /*ASUS-BBSP Trigger modem core dump when invalid PDU signature+*/
#include <asm/intel-mid.h> /*ASUS-BBSP Trigger modem core dump when invalid PDU signature+*/
...
static void dlp_trace_complete_rx(struct hsi_msg *msg)
{
	...
	if (msg->status != HSI_STATUS_COMPLETED) {
		...
		if (EDLP_ASUS_DEBUG_REPORT) {
			pr_err (DRVNAME": [ASUS Debug]Trigger modem Core Dump! reason:Invalid PDU status: %d (ignored)\n", msg->status);
