...
#include <linux/gpio.h> /*ASUS-BBSP Trigger modem core dump when AP side received invalid PDU signature & size on TTY RX+*/
#include <asm/intel-mid.h> /*ASUS-BBSP Trigger modem core dump when AP side received invalid PDU signature & size on TTY RX+*/
#include <linux/tty.h>
 ...
 */
static void *g_hsi_buff;/*ASUS-BBSP Prealloc memory for hsi+*/
static int g_dbg_cnt;/*ASUS-BBSP Prealloc memory for hsi+*/
void *dlp_buffer_alloc(unsigned int buff_size, dma_addr_t *dma_addr)
{
	void *buff = NULL;/*ASUS-BBSP Prealloc memory for hsi+*/
	int flags = in_interrupt() ? GFP_ATOMIC : GFP_KERNEL;
	if (dlp_drv.is_dma_capable) {

		/*ASUS-BBSP Prealloc memory for hsi+++*/
		/* if buff size  64K - 256K using pre alloc memory */
		if ((buff_size > 65535 && buff_size <= 262144) && g_hsi_buff != NULL && g_dbg_cnt == 0) {
			g_dbg_cnt++;
			return g_hsi_buff;
		}
		/* if buff size  64K - 256K pre alloc 256K for used */
		if ((buff_size > 65535 && buff_size <= 262144) && g_hsi_buff == NULL) {
			buff_size = 262144;
		}
		/*ASUS-BBSP Prealloc memory for hsi---*/
		...
		if (buff_size == 262144) {
			if (g_hsi_buff == NULL) {
				g_hsi_buff = buff;
				printk("=== save buff %p", buff);
			} else {
				/* it shouldn't be here */
				printk("===***prealloc buffer has something wrong g_dbg_cnt %d***===", g_dbg_cnt);
				return buff;
			}
			g_dbg_cnt++;
		}
		...
		struct hsi_msg *pdu)
{
	...
	if (DLP_HEADER_VALID_SIGNATURE(header[0])) {
		...
		/*ASUS-BBSP Trigger modem core dump when invalid PDU signature+++*/
		if (EDLP_ASUS_DEBUG_REPORT) {
			pr_err (DRVNAME": [ASUS Debug]Trigger modem Core Dump! reason:Invalid PDU signature 0x%x\n", header[0]);
			gpio_direction_output (get_gpio_by_name("AP_REQ_CORE_DUMP"), 1);
			mdelay (10);
			gpio_direction_output (get_gpio_by_name("AP_REQ_CORE_DUMP"), 0);
		}
		/*ASUS-BBSP Trigger modem core dump when invalid PDU signature---*/
...
void dlp_pdu_update(struct dlp_channel *ch_ctx, struct hsi_msg *pdu)
{
	...
	if ((!pdu->actual_len) || (pdu->actual_len > pdu_size)) {
		...
		/*ASUS-BBSP Trigger modem core dump when AP side received invalid PDU signature & size on TTY RX+++*/
		if (EDLP_ASUS_DEBUG_REPORT) {
			print_hex_dump(KERN_DEBUG,
				DRVNAME"DEBUG_TTY", DUMP_PREFIX_ADDRESS,
				16, 4,
				sg_virt(pdu->sgt.sgl), 64, 1);
			pr_err (DRVNAME": [ASUS Debug]Trigger modem Core Dump! reason:Invalid pdu size\n");
			gpio_direction_output (get_gpio_by_name("AP_REQ_CORE_DUMP"), 1);
			mdelay (10);
			gpio_direction_output (get_gpio_by_name("AP_REQ_CORE_DUMP"), 0);
		}
...
static int dlp_driver_probe(struct device *dev)
{
	...
	printk("[Progress][hsi-dlp] Probe starts\n");/*ASUS-BBSP Add hsi-dlp probe logs+*/
	...
	dlp_create_debug_fs();
	printk("[Progress][hsi-dlp] Probe ends\n");/*ASUS-BBSP Add hsi-dlp probe logs+*/
	return 0;
