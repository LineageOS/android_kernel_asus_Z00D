...
int wakeup_print_data; /* ASUS-BBSP+ packet wakeup trace */
/*
 *
 * LOCAL functions
 *
...
static void dlp_net_complete_rx(struct hsi_msg *pdu)
{
	...
	do {
		...
		/* ASUS-BBSP packet wakeup trace +++ */
		if (wakeup_print_data) {
			wakeup_print_data = 0;
			if (skb->protocol == htons(ETH_P_IP)) {
				if (data_size >= 16) {
					pr_warn("first packet from wakeup ipv4 packet src ip %d.%d.%d.%d\n", *(data_addr+12), *(data_addr+13), *(data_addr+14), *(data_addr+15));
				}
				/*
				if (data_size >= 64) {
				...
				*/
				...
				pr_warn("first packet from wakeup not ipv4 packet \n");
				/*
				if (data_size >= 64) {
					print_hex_dump(KERN_WARNING,
						DRVNAME"_NET_RX", DUMP_PREFIX_OFFSET,
