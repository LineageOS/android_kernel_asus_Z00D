				  ...
				  struct v4l2_mbus_framefmt *fmt)
{
	...
	ret = gc2155_write_reg_array(c, gc2155_common);
	if (ret)
		pr_info("%s: write seq gc2155_common fail\n", __func__);
...
int gc2155_s_controlDarkMode(struct v4l2_subdev *sd, int DarkMode)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	printk("[DEBUG] %s DarkMode status = %d, start\n", __func__, DarkMode);
...
int gc2155_q_controlDarkMode(struct v4l2_subdev *sd, u16 value)
{
	return 0;
}
static struct gc2155_control gc2155_controls[] = {
	...
	},
	{
		...
		.tweak = gc2155_s_controlDarkMode,
		.query = gc2155_q_controlDarkMode,
	},
...
#if 0  /*remove stream off to decrease probe time*/
			...
			struct v4l2_streamparm *param)
{
	...
	int run_mode = param->parm.capture.capturemode;

	switch (run_mode) {
	case CI_MODE_VIDEO:
			gc2155_res = gc2155_res_video;
			n_res = N_RES_VIDEO;
			pr_info("%s gc2155_res_video\n", __func__);
			break;
	case CI_MODE_STILL_CAPTURE:
			gc2155_res = gc2155_res_still;
			n_res = N_RES_STILL;
			pr_info("%s gc2155_res_still\n", __func__);
			break;
	default:
			gc2155_res = gc2155_res_preview;
			n_res = N_RES_PREVIEW;
			pr_info("%s gc2155_res_preview\n", __func__);
	}
				...
				const struct i2c_device_id *id)
{
	...
	if (HW_ID >= 3) {  /*for PR HW change PCB_ID2*/
		...
		if (pcb_value) {
			pr_info("%s PCB_ID2(%d) not match\n", __func__, pcb_value);
			return 1;
		}
	} else if (ATD_hm2051_status) {
		pr_info("%s hm2051 Probe success do not run GC2155\n", __func__);
		return 1;
	}
