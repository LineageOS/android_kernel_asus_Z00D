...
/* ASUS_BSP +++ bill_chen "add ISP input and output first frame debug" */
bool g_ISP_input_first_frame;
bool g_ISP_output_first_frame;
/* ASUS_BSP --- bill_chen "add ISP input and output first frame debug" */
...
int atomisp_css_rx_err_ecc_no_err_check(struct atomisp_device *isp)
{
	...
	if (recheck++ ==  0) {
		pr_info("Detected: delay for one more time recheck = %d\n", recheck);
		return 1;
	} else {
...
irqreturn_t atomisp_isr(int irq, void *dev)
{
	...
	if (irq_infos & CSS_IRQ_INFO_CSS_RECEIVER_SOF) {
/* ASUS_BSP +++ bill_chen "add ISP input and output first frame debug" */
		if (true == g_ISP_input_first_frame) {
			pr_info("%s CSS_IRQ_INFO_CSS_RECEIVER_SOF +\n", __func__);
			g_ISP_input_first_frame = false;
		}
/* ASUS_BSP --- bill_chen "add ISP input and output first frame debug" */
...
void atomisp_set_term_en_count(struct atomisp_device *isp)
{
...
#else
	val |= 0x20 << TERM_EN_COUNT_1LANE_OFFSET;
#endif
	...
	val |= 0x1A << (pwn_b0 ? TERM_EN_COUNT_4LANE_PWN_B0_OFFSET :
				TERM_EN_COUNT_4LANE_OFFSET);
#else
	val |= 0x19 << (pwn_b0 ? TERM_EN_COUNT_4LANE_PWN_B0_OFFSET :
				TERM_EN_COUNT_4LANE_OFFSET);
#endif
...
int atomisp_ospm_dphy_up(struct atomisp_device *isp)
{
	...
	g_ISP_input_first_frame = true; /* ASUS_BSP bill_chen "add ISP input and output first frame debug" */
	g_ISP_output_first_frame = true; /* ASUS_BSP bill_chen "add ISP input and output first frame debug" */
	spin_unlock_irqrestore(&isp->lock, flags);
