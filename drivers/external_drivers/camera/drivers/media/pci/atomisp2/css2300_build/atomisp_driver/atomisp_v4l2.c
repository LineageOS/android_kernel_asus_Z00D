...
/* ASUS_BSP +++ bill_chen add atomisp allocate memory workqueue */
#include <linux/workqueue.h>
struct work_struct create_hmm_wq;
static struct workqueue_struct *create_hmm_workqueue;

static void atomisp_create_hmm_work(struct work_struct *work)
{
	int err;

	err = hmm_pool_register(repool_pgnr, HMM_POOL_TYPE_RESERVED);
	if (err)
		pr_err("%s Failed to register reserved memory pool.\n", __func__);

	return;
}

void atomisp_create_hmm_workqueue(void){
	INIT_WORK(&create_hmm_wq, atomisp_create_hmm_work);
	create_hmm_workqueue = create_singlethread_workqueue("atomisp_create_hmm_wq");
}

void atomisp_destroy_hmm_workqueue(void){
	destroy_workqueue(create_hmm_workqueue);
}
/* ASUS_BSP --- bill_chen add atomisp allocate memory workqueue */
...
#else
	queue_work(create_hmm_workqueue, &create_hmm_wq);
#endif
/* ASUS_BSP --- bill_chen add atomisp allocate memory workqueue */
...
{
	atomisp_create_hmm_workqueue();    /* ASUS_BSP bill_chen add atomisp allocate memory workqueue */
	return pci_register_driver(&atomisp_pci_driver);
}

static void __exit atomisp_exit(void)
{
	atomisp_destroy_hmm_workqueue();    /* ASUS_BSP bill_chen add atomisp allocate memory workqueue */
