...
extern bool read_lcd_calibration_data(void);
int psb_brightness;
...
int psb_set_brightness(struct backlight_device *bd)
{
	...
	if (!bl_cali_complete && level != BRIGHTNESS_INIT_LEVEL) {
		err = read_lcd_calibration_data();
		if (!err)
			printk("%s:: [DISPLAY]Read Lcd Calibration Data Fail\n", __func__);
		bl_cali_complete = true;
	}
