...
/*	ASUS_BSP: Louis ++	*/
extern void force_power_on_dpi_panel(struct drm_psb_private *dev_priv);
extern void force_power_off_dpi_panel(struct drm_psb_private *dev_priv);
struct psb_framebuffer *g_psbfb;
/*	ASUS_BSP: Louis --	*/
...
/* ASUS_BSP: Louis	+++ */
void psbfb_panel_blank(struct drm_psb_private *dev_priv, int blank)
{
	struct fb_info *info = g_psbfb->fbdev;
	...
	if (!blank) {
		...
		psbfb_blank(FB_BLANK_UNBLANK, info);
#endif
		force_power_on_dpi_panel(dev_priv);
		...
		psbfb_blank(FB_BLANK_POWERDOWN, info);
#endif
		force_power_off_dpi_panel(dev_priv);
	}
}

void psbfb_memclear(void)
{
	char *fb_screen_base = NULL;
	struct fb_info *fbi = g_psbfb->fbdev;

	fb_screen_base = ioremap(fbi->fix.smem_start, fbi->fix.smem_len);

	DRM_INFO("[Display] %s: allocating %lu bytes at %p (%lx physical) for fb\n", __func__, fbi->fix.smem_len, fb_screen_base, fbi->fix.smem_start);

	memset(fb_screen_base, 0x0, fbi->fix.smem_len);
}
/* ASUS_BSP: Louis	--- */
...
static int psbfb_create(struct psb_fbdev * fbdev, struct drm_fb_helper_surface_size * sizes)
{
	...
	*/
	...
	g_psbfb = psbfb;	/*	ASUS_BSP: Louis, keep fb info	*/
