...
struct list_head	asus_meminfo_head;
void add_meminfo(struct drm_device *dev, IMG_HANDLE hKernelMemInfo)
{
	struct asus_meminfo *tmp;
	tmp =  kmalloc(sizeof(struct asus_meminfo), GFP_KERNEL);
	tmp->dev = dev;
	tmp->hKernelMemInfo = hKernelMemInfo;
	tmp->tgid = psb_get_tgid();
	INIT_LIST_HEAD(&tmp->list);
	list_add(&tmp->list, &asus_meminfo_head);
}
void delete_meminfo(struct drm_device *dev, IMG_HANDLE hKernelMemInfo)
{
	struct asus_meminfo *tmp ;
	struct list_head *iterator, *n;
	list_for_each_safe(iterator, n, &asus_meminfo_head) {
		tmp = list_entry(iterator, struct asus_meminfo, list);
		if (tmp->hKernelMemInfo == hKernelMemInfo) {
			list_del(&tmp->list);
			kfree(tmp);
		}
	}
...
void delete_all_meminfo(void){
	struct asus_meminfo *tmp ;
	struct list_head *iterator, *n;
	delete_all_map = 1;
	list_for_each_safe(iterator, n, &asus_meminfo_head) {
		tmp = list_entry(iterator, struct asus_meminfo, list);
		global_tgid = tmp->tgid;
		psb_gtt_unmap_meminfo(tmp->dev, tmp->hKernelMemInfo);
		list_del(&tmp->list);
		printk ("delete_all_meminfo \n");
		kfree(tmp);
	}
	delete_all_map = 0;
}
...
int psb_gtt_init(struct psb_gtt *pg, int resume)
{
	...
	INIT_LIST_HEAD(&asus_meminfo_head);

	if (list_empty(&asus_meminfo_head) == true) {
		printk(KERN_INFO "[Wei] asus_meminfo_head is NULL\n");
	}
			       ...
			       struct drm_mm_node **node)
{
	...
	ret = psb_gtt_mm_get_ht_by_pid_locked(mm, tgid, &hentry);
	if (ret) {
		DRM_INFO("Cannot find entry for pid %d\n", tgid);
		spin_unlock(&mm->lock);
		return ret;
	}
	spin_unlock(&mm->lock);
	...
	*node = tmp;
	...
	if (!hentry->count) {
		DRM_INFO("count of mapping entry is zero, tgid=%d\n", tgid);
			...
			uint32_t *offset)
{
	...
	ret = psb_get_meminfo_by_handle(dev_priv, hKernelMemInfo,
			&psKernelMemInfo);
	if (ret) {
		DRM_INFO("Cannot find kernelMemInfo handle %d\n",
			  (int)hKernelMemInfo);
		return -EINVAL;
	}
...
int psb_gtt_unmap_meminfo(struct drm_device *dev, IMG_HANDLE hKernelMemInfo)
{
	return psb_gtt_unmap_common(dev,
				delete_all_map ? global_tgid : psb_get_tgid(),
			      ...
			      struct drm_file *file_priv)
{
	...
	case PSB_GTT_MAP_TYPE_MEMINFO:
		ret = psb_gtt_map_meminfo(dev,
				arg->hKernelMemInfo,
				page_align,
				offset_pages);
		if (!ret) {
			add_meminfo(dev, arg->hKernelMemInfo);
		}
		return ret;
	case PSB_GTT_MAP_TYPE_VIRTUAL:
		return psb_gtt_map_vaddr(dev,
					...
					offset_pages);
/*ASUS BSP Wei +++*/
	case PSB_GTT_MAP_FREE_ALL_MAP:
		delete_all_meminfo();
		DRM_INFO("PSB_GTT_MAP_FREE_ALL_MAP release all mmap memory\n");
		return 0;
/*ASUS BSP Wei ---*/
	default:
				...
				struct drm_file *file_priv)
{
	...
	switch (type) {
	case PSB_GTT_MAP_TYPE_MEMINFO:
		ret = psb_gtt_unmap_meminfo(dev, arg->hKernelMemInfo);
		if (!ret)
			delete_meminfo(dev, arg->hKernelMemInfo);
		return ret;
	case PSB_GTT_MAP_TYPE_VIRTUAL:
