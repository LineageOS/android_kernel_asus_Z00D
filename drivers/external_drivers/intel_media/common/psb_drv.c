...
unsigned int raw_luminance;
EXPORT_SYMBOL(raw_luminance);
unsigned int cali_luminance;
EXPORT_SYMBOL(cali_luminance);
...
#ifdef CONFIG_SUPPORT_OTM1284A_MIPI_720X1280_DISPLAY
extern struct platform_driver ze500cl_vid_lcd_driver;
#endif
	...
	for (pipe = 0; pipe < PSB_NUM_PIPE; pipe++) {
		...
		if (interval > 0 || nanosec_rem > 200000000) {
			...
			if (pipe == 0) {
				...
				/*austin+++*/
				if ((interval * 1000 + nanosec_rem/1000000) > 3000) {
					DRM_INFO("pipe %d vsync te missing > 3000ms reset it !\n\n", pipe);
					mdfld_reset_same_dpi_panel(dev_priv);/*austin---*/
				}
			}
		}
	}
...
static int lcd_raw_luminance_read(struct file *file, char __user *buffer,
				    size_t count, loff_t *ppos)
{
	int len = 0;
	ssize_t ret = 0;
	...
	len += sprintf(buff, "===============\nPanel Vendor: %s \nRaw Luminance: %d(nits) \nLuminance after Calibration: %d(nits)\n===============\n", asus_panel_id == 1 ? "AUO" : "TM", raw_luminance, cali_luminance);
	ret = simple_read_from_buffer(buffer, count, ppos, buff, len);
	kfree(buff);
...
static int lcd_raw_luminance_write(struct file *file, const char *buffer,
			  size_t count, loff_t *ppos)
{
	return 0;
}
...
static const struct file_operations psb_raw_luminance_proc_fops = {
       .owner = THIS_MODULE,
       .read = lcd_raw_luminance_read,
       .write = lcd_raw_luminance_write,
};

static int __init psb_init(void)
{
	int ret;
	struct proc_dir_entry *lcd_unique_id_node;
	struct proc_dir_entry *lcd_raw_luminance_node;
...
#ifdef CONFIG_SUPPORT_OTM1284A_MIPI_720X1280_DISPLAY
    ret = platform_driver_register(&ze500cl_vid_lcd_driver);
#endif

	lcd_unique_id_node = proc_create(LCD_UNIQUE_ID_PROC_ENTRY, 0444, NULL, &psb_lcd_unique_id_proc_fops);
	if (!lcd_unique_id_node) {
		DRM_ERROR("Unable to create %s\n", LCD_UNIQUE_ID_PROC_ENTRY);
		return -EINVAL;
	}

	lcd_raw_luminance_node = proc_create(LCD_RAW_LUMI_PROC_ENTRY, 0444, NULL, &psb_raw_luminance_proc_fops);
	if (!lcd_raw_luminance_node) {
		DRM_ERROR("Unable to create %s\n", LCD_RAW_LUMI_PROC_ENTRY);
		return -EINVAL;
	}
