...
static const drm_psb_reg_info g_GVD_REGS[] = {
	{0x2060, "GFX_GVD_CG_DIS"},
	{0x2064, "VED_CG_DIS"},
	{0x2068, "VEC_CG_DIS"},
	{0x2084, "IIR_RW"},
	{0x20A0, "IER"},
...
static void pnw_topaz_dump_GVD(struct drm_psb_private *dev_priv)
{
	u32 reg_val, i;
	if (psb_get_power_state(OSPM_VIDEO_ENC_ISLAND) == 1) {
		for (i = 0; i < 2; i++) {
			DRM_ERROR("Topaz core %d registers:\n");
			topaz_read_core_reg(dev_priv, i,
					TOPAZ_MTX_PC, &reg_val);
			DRM_ERROR("\tPC pointer: 0x%08x\n", reg_val);
			TOPAZ_READ32(0x001C, &reg_val, i);
			DRM_ERROR("\tMMU status: 0x%08x\n", reg_val);
			TOPAZ_READ32(0x0058, &reg_val, i);
			DRM_ERROR("\tFiFo status: 0x%08x\n", reg_val);
			MVEA_READ32(0x0018, &reg_val, i);
			DRM_ERROR("\tMVEA status: 0x%08x\n", reg_val);
			MTX_READ32(0x0008, &reg_val, i);
			DRM_ERROR("\tMTX status 0x%08x\n", reg_val);
		}

		TOPAZ_MULTICORE_READ32(TOPAZSC_CR_MULTICORE_CMD_FIFO_1,
				&reg_val);
		reg_val &= MASK_TOPAZSC_CR_CMD_FIFO_SPACE;
		DRM_ERROR("TOPAZ: Free words in command FIFO %d\n", reg_val);
	} else {
		DRM_ERROR("Can't dump Topaz register. It's powered off.\n");
		return;
	}

	for (i = 0; i < ARRAY_SIZE(g_GVD_REGS); i++)
		DRM_ERROR("GVD REG %s:0x%08x",
				g_GVD_REGS[i].reg_name,
				PSB_RVDC32(g_GVD_REGS[i].offset));
}
