	...
	struct akm_compass_data *akm, char const *buf, size_t count, int pos)
{
	...
	printk("[E-compass] Setting compass delay : %d\n", val);
...
static int Compass_sensor_I2C_stress_test_function()
{	
	uint8_t buffer[AKM_SENSOR_DATA_SIZE];
	int err = 0;
...
static DEVICE_ATTR(akm09911_debug_state, 0664, get_akm09911_debug_state, Set_akm09911_debug_state);
...
*/
	...
	err = gpio_request(pdata->gpio_RSTN, "ECOMP_RST#");
	if (err)
		pr_err("[E-compass] %s: unable to request gpio %d (ECOMP_RST#)\n", __func__, pdata->gpio_RSTN);

	err = gpio_direction_output(pdata->gpio_RSTN, 1);
	if (err < 0)
